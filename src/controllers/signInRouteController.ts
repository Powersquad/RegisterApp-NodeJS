...
export const start = async (req: Request, res: Response): Promise<void> => {
    return EmployeeExistsQuery.query()
        .then((employeeExistsCommandResponse: CommandResponse<boolean>):void => {
            if((employeeExistsCommandResponse.data == null) || !employeeExistsCommandResponse.data){
            ...
            return res.render(
                ViewNameLookup.SignIn, <PageResponse>{
...
export const signIn = async (req: Request, res: Response): Promise<void> => {
    return EmployeeSignIn.execute(req.body,req.session)
        .then((): void => {
            ...
            console.error(
                "Invalid credentials when attempting to sign in. " + error.message);
...
export const clearActiveUser = async (req: Request, res: Response): Promise<void> => {
    if(req.session == null){
        res.status(204).send(<ApiResponse>{ redirectUrl: RouteLookup.SignIn});
        return;
    }
